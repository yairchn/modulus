#!/bin/bash
#SBATCH --account=<account being used>
#SBATCH --qos=regular
...
cd ${HOME}/NV-dlesm/
srun -n 4 shifter --env PYTHONUSERBASE=${python_base}  --entrypoint python3 -u train.py --config-name=${config} experiment_name=${expmt_name} output_dir=${expmt_dir} batch_size=4
